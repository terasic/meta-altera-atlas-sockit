		...
		if (dev->fft256_buffer_inflight_count >= 2) {
			...
			do {
				dma_status =
				    ioread32(dev->ioremap_sgdma_from_fft_csr +
					     CSR_STATUS_REG);
			} while ((dma_status & ALTERA_MSGDMA_CSR_BUSY_MASK) !=
				 0);
		}
				...
				loff_t *offset)
{
	...
	} while ((dma_status & ALTERA_MSGDMA_CSR_BUSY_MASK) != 0);
...
static int platform_probe(struct platform_device *pdev)
{
	...
	if ((dma_status &
	     (ALTERA_MSGDMA_CSR_BUSY_MASK | ALTERA_MSGDMA_CSR_STOP_STATE_MASK |
	...
	if ((dma_status &
	     ALTERA_MSGDMA_CSR_DESCRIPTOR_BUFFER_EMPTY_MASK) == 0) {
		pr_err("sgdma_from_fft\n");
	...
	if ((dma_control &
	     (ALTERA_MSGDMA_CSR_STOP_MASK | ALTERA_MSGDMA_CSR_RESET_MASK |
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Rod Frazer <rfrazer@altera.com>");
